  ...
  try {
    ...
    try {
      created = await apiRequest<Prompt>(
        ...
        jsonRequestOptions("POST", { name: enteredName.trim(), content: unwrapFence(innerText) }),
      );
    } catch (error) {
    ...
    if (editorState.currentPrompt) {
      ...
      await apiRequest(
        promptApiPath(editorState.currentPrompt.folder, editorState.currentPrompt.name),
        jsonRequestOptions("PUT", { content: editorState.currentPrompt.content }),
      );
      setSaveStatus("");
    }
